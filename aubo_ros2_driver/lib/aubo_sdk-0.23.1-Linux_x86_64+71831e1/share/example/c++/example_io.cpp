...
void exampleStaticLinkIO(RpcClientPtr cli)
{
    ...
    // 将十进制数转换为二进制数
    std::bitset<32> input_value_binary(input_value_decimal);
    // 将std::bitset<32>转换为字符串
    std::string input_value_string = input_value_binary.to_string();
    // 反转二进制数
    std::reverse(input_value_string.begin(), input_value_string.end());

    std::cout << "所有联动IO输入的值: "
              << input_value_string.substr(0, link_input_num) << std::endl;

    // 获取联动IO输出的值
    int output_value_decimal = robot->getIoControl()->getStaticLinkOutputs();
    // 将十进制数转换为二进制数
    std::bitset<32> output_value_binary(output_value_decimal);
    // 将std::bitset<32>转换为字符串
    std::string output_value_string = output_value_binary.to_string();
    // 反转二进制数
    std::reverse(output_value_string.begin(), output_value_string.end());

    std::cout << "所有联动IO输出的值: "
              << output_value_string.substr(0, link_output_num) << std::endl;
}
