...
#define ENUM_AuboErrorCodes_DECLARES                                       \
    ...
    ENUM_ITEM(AUBO_NO_ACCESS, 7, "无法访问")                               \
    ENUM_ITEM(AUBO_CONN_REFUSED, 8, "连接被拒绝")                          \
    ENUM_ITEM(AUBO_CONN_RESET, 9, "连接被重置")                            \
    ...
    ENUM_ITEM(AUBO_DIMENSION_ERR, 16, "输入参数维数不对")                  \
    ENUM_ITEM(AUBO_SINGULAR_ERR, 17, "输入的配置可能会奇异")               \
    ENUM_ITEM(AUBO_POS_BOUND_ERR, 18, "输入的位置边界超过极限范围")        \
    ENUM_ITEM(AUBO_INIT_POS_ERR, 19, "初始位置输入不合理")                 \
    ENUM_ITEM(AUBO_ELP_SETTING_ERR, 20, "包络体设置错误")                  \
    ENUM_ITEM(AUBO_TRAJ_GEN_FAIL, 21, "轨迹生成失败")                      \
    ENUM_ITEM(AUBO_TRAJ_SELF_COLLISION, 22, "轨迹自碰撞")                  \
    ENUM_ITEM(AUBO_IK_NO_CONVERGE, 23, "逆解计算不收敛，计算出错")         \
    ENUM_ITEM(AUBO_IK_OUT_OF_RANGE, 24, "逆解计算超出机器人最大限制")      \
    ENUM_ITEM(AUBO_IK_CONFIG_DISMATCH, 25, "逆解输入配置存在错误")         \
    ENUM_ITEM(AUBO_IK_JACOBIAN_FAILED, 26, "逆解雅可比矩阵计算失败")       \
    ENUM_ITEM(AUBO_IK_NO_SOLU, 27, "目标点存在解析解，但均不满足选解条件") \
    ENUM_ITEM(AUBO_IK_UNKOWN_ERROR, 28, "逆解返回未知类型错误")            \
...
struct PayloadCalibResult
{
    double mass;                 // 质量
    std::vector<double> com;     // 质心
    std::vector<double> inertia; // 惯量
    int error_code;              // 错误码
};

inline std::ostream &operator<<(std::ostream &os, PayloadCalibResult p)
{
    return os;
}

struct PayloadCalibData
{
    std::vector<double> q;       // 关节角
    std::vector<double> qd;      // 关节速度
    std::vector<double> current; // 关节电流
};

inline std::ostream &operator<<(std::ostream &os, PayloadCalibData p)
{
    return os;
}
